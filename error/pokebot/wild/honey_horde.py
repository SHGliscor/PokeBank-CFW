...
def require_honey(br, *, first_active_slot: bool = False) -> dict:
    ...
    # ORAS can legitimately expose multiple Honey stacks in the save-backed
    # Items pocket.  Treat the first active Honey stack as the usable stack;
    # do not Safety-HOLD merely because duplicate Honey entries exist.
    row = dict(honey[0])
...
def honey_active_index(authority: dict) -> int:
    # Multiple Honey stacks are valid. Use the first active Honey entry as the
    # same slot-1 authority established by require_honey().
    for index, row in enumerate(authority["entries"]):
        if int(row["item_id"]) == HONEY_ITEM_ID and int(row["quantity"]) > 0:
            return int(index)
    raise RuntimeError("FIELD BAG: no active Honey entry found")
