...
def read_balls_state(br, cursor_reader) -> dict:
    ...
    HF101b: after a failed Horde shiny throw the Bag-state-1 handoff can briefly
    reopen the Balls pocket with a transient cursor+0x14 value that matches
    neither page-local 0..5 nor a self-consistent global index (hardware example
    2026-09-22: page=0 raw=8 entry_count=5).  Retry the full read a few times
    so a settling UI does not force SHINY_AUTO_CATCH_SAFETY_HOLD.  Still fail
    closed if the encoding never stabilizes — never guess a cursor cell.
    """
    last_exc: Exception | None = None
    for attempt in range(8):
        try:
            return _read_balls_state_once(br, cursor_reader)
        except RuntimeError as exc:
            msg = str(exc)
            # Only retry the known transient encoding / page races after a
            # failed throw. Other authority failures stay immediate.
            if (
                "unexpected Balls-pocket cursor+0x14 encoding" in msg
                or "unexpected Balls-pocket page=" in msg
                or "Balls-pocket authority is not active" in msg
            ):
                last_exc = exc
                time.sleep(0.08 + 0.04 * attempt)
                continue
            raise
...
def _read_balls_state_once(br, cursor_reader) -> dict:
    """Single-shot Balls-pocket read; used by read_balls_state with retries."""
    ...
    #
    # Hardware 2026-09-22 (HF100 support ZIP): page=0 raw=8 entry_count=5 —
    # raw is outside both encodings.  That is treated as transient; the outer
    # read_balls_state retries until PAGE_LOCAL / GLOBAL_INDEX_NORMALIZED
    # appears or the retry budget is exhausted.
