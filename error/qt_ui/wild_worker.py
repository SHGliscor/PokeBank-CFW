...
def _atomic_write(path: Path, payload):
    ...
    HF101: a transient PermissionError on tmp.replace (antivirus, OneDrive,
    Explorer, second instance) previously became a false SAFETY_HOLD on an
    otherwise valid non-shiny Horde. Retry a few times, then fall back to a
    direct write so stats persistence cannot stop the hunt.
    """
    ...
    for attempt in range(6):
        try:
            ...
            last_exc = exc
            _time.sleep(0.05 * (attempt + 1))
    # Final best-effort non-atomic write so the hunt continues.
    try:
...
class WildHuntWorker(QObject):
    ...
    def _save_last_seen_entry(self, item):
        ...
        HF101e: use resilient _atomic_write so a Windows lock cannot abort the
        emit loop mid-Horde (which previously left Recent encounters at 4/5).
        """
        history = self._load_last_seen()
        history.insert(0, dict(item))
        # Keep a full Horde (5) plus prior context for restart restore.
        history = history[:12]
        try:
            _atomic_write(self.last_seen_path, history)
        except Exception as exc:
            self._log(
                f"LAST SEEN WRITE FAILED: {type(exc).__name__}: {exc} • "
                "UI feed still updated"
            )
    ...
    def _commit_nonshiny_horde_seen(self, payloads):
        ...
        # Horde = five independent shiny rolls.  The phase counter is kept as
        # one Horde encounter for rate/session reporting, but shiny probability
        # must advance once for each of the five Pokémon that were actually
        # inspected.  This is deliberately statistics-only and cannot affect
        # Horde input/control.
        ...
        if detected_charm is not None:
            ...
            for _ in range(len(payloads)):
                current_log = advance_phase_log_miss(current_log, odds.probability)
            self.lifetime["phase_log_miss"] = current_log
            self.lifetime["phase_cumulative_probability"] = cumulative_probability_from_log_miss(current_log)
        else:
            self.lifetime["phase_log_miss"] = None
            self.lifetime["phase_cumulative_probability"] = None
    ...
    def _honey_quantity(snapshot):
        ...
        # Multiple Honey stacks are valid. The Horde trigger uses the first
        # active Honey stack, matching require_honey()/active slot 1 authority.
        return int(rows[0].get("quantity") or 0) if rows else None
        ...
        try:
            ...
            while True:
                ...
                for payload in emit_order:
                    ...
                    # Never let a ledger write abort the remaining UI emits.
                    try:
                        self._save_last_seen_entry(payload)
                    except Exception as exc:
                        self._log(
                            f"LAST SEEN SAVE SKIP: {type(exc).__name__}: {exc}"
                        )
                    self.encounter.emit(payload)
                    self.last_seen_entry.emit(payload)
                self._log(
                    f"LAST SEEN FEED: emitted {len(emit_order)} Horde/wild "
                    f"payload(s) to Recent encounters "
                    f"(shiny={sum(1 for p in emit_order if p.get('is_shiny'))})"
                )
                ...
                if horde_seen_committed:
                    # HF101: push every Horde member into the live Recent
                    # encounters feed (newest-first) so the dashboard shows
                    # the last 5 Pokémon seen, not a stale single-encounter list.
                    # Emit in reverse slot order so the final visible top row is
                    # the primary (slot 0) after successive insertRow(0).
