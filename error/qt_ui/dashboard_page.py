...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QAbstractItemView, QSizePolicy, QProgressBar, QDialog, QTabWidget
)

from .widgets import Panel, DataPair, StarterRow, PartyCard, SpriteLoader, LastSeenFiveSlots
...
HISTORY_ROW_HEIGHT = 36
HISTORY_HEADER_HEIGHT = 28
# Generous frame allowance for QSS borders/padding — must fit 5 rows cleanly.
HISTORY_TABLE_HEIGHT = HISTORY_HEADER_HEIGHT + HISTORY_VISIBLE_ROWS * HISTORY_ROW_HEIGHT + 20
HISTORY_TABS_MIN_HEIGHT = HISTORY_TABLE_HEIGHT + 40  # tab bar chrome
HISTORY_PANEL_MIN_HEIGHT = HISTORY_TABS_MIN_HEIGHT + 40  # panel title + margins
...
class DashboardPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.hunt_info_panel = hunt_info
        # HF102b: hide Current hunt on the dashboard — session/right column
        # already covers live counters; frees vertical space for 5 history slots.
        hunt_info.setVisible(False)
        ...
        history_panel = Panel("History — Fixed 5-slot Recent encounters")
        self.history_tabs = QTabWidget()

        def _history_table():
            # HF101: 13 columns — sprite + Species name so the last 5 Pokémon
            # are identifiable without relying only on the 27x27 icon.
            table = QTableWidget(0, 13)
            table.setObjectName("LastSeenTable")
            table.setHorizontalHeaderLabels([
                "", "Species", "Nature", "Ability", "Evolution",
                "HP", "ATK", "DEF", "SPA", "SPD", "SPE", "SUM", "SV"
            ])
            ...
            # Always reserve vertical space for exactly 5 rows (never shrink).
            table.setMinimumHeight(HISTORY_TABLE_HEIGHT)
            table.setFixedHeight(HISTORY_TABLE_HEIGHT)
            table.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
            table.verticalHeader().setDefaultSectionSize(HISTORY_ROW_HEIGHT)
            table.verticalHeader().setMinimumSectionSize(HISTORY_ROW_HEIGHT)
            table.verticalHeader().setMaximumSectionSize(HISTORY_ROW_HEIGHT)
            ...
            # Sprite | Species | Nature | Ability (stretch) | Evolution | IVs | SUM | SV
            ...
            for col in range(5, 11):
                table.setColumnWidth(col, 26)
            table.setColumnWidth(11, 36)
            table.setColumnWidth(12, 42)
            return table

        self._recent_shiny_items = []
        # HF102: fixed 5-slot list (not QTableWidget) so last 5 always show.
        self.last_seen_slots = LastSeenFiveSlots()
        self.recent_oras_table = _history_table()
        self.recent_target_table = _history_table()
        self.history_tabs.addTab(self.last_seen_slots, "Recent encounters")
        ...
        # Tab bar (~32) + header (26) + 5 rows + small padding
        slots_h = LastSeenFiveSlots.ROW_HEIGHT * LastSeenFiveSlots.SLOT_COUNT + 26
        tabs_h = slots_h + 36
        self.history_tabs.setMinimumHeight(tabs_h)
        self.history_tabs.setSizePolicy(
            self.history_tabs.sizePolicy().horizontalPolicy(),
            __import__("PySide6.QtWidgets", fromlist=["QSizePolicy"]).QSizePolicy.MinimumExpanding,
        )
        history_panel.setMinimumHeight(tabs_h + 48)
        history_panel.outer.addWidget(self.history_tabs, 1)
        hist_caption = QLabel("Always 5 fixed rows · newest on top · empty slots show — · backend unchanged")
        hist_caption.setObjectName("FieldLabel")
        hist_caption.setStyleSheet("color:#6f757b; font-size:9px;")
        history_panel.outer.addWidget(hist_caption)
        ...
        center.addWidget(history_panel, 3)
    ...
    def _add_history_row(self, table, data, *, force_shiny=None):
        ...
        for _r in range(table.rowCount()):
            table.setRowHeight(_r, HISTORY_ROW_HEIGHT)
        ...
        table.setItem(0, 4, evolution_item)

        for offset, value in enumerate(vals, start=5):
    ...
    def add_last_seen(self, data):
        reqs = self.last_seen_slots.prepend(data)
        self._request_slot_sprites(reqs)

    def set_last_seen_history(self, entries):
        # entries newest-first; slots widget stores newest-first.
        reqs = self.last_seen_slots.set_entries(list(entries or [])[:HISTORY_VISIBLE_ROWS])
        self._request_slot_sprites(reqs)

    def _request_slot_sprites(self, reqs):
        for token, species_id, shiny in reqs or []:
            self.last_seen_sprite_loader.request_sprite(token, species_id, shiny)
    ...
    def _last_seen_sprite_ready(self, attempt_token, pixmap):
        if pixmap is None or pixmap.isNull():
            return
        if self.last_seen_slots.apply_sprite(attempt_token, pixmap):
            return
        for table in (self.recent_oras_table, self.recent_target_table):
    ...
    def set_target_phase_data(self, phase_data):
        for key in (
            ...
            "wild_horde", "wild_cave", "wild_surf", "wild_fishing",
        ):
            ...
            self.target_phases[key] = {
                ...
                "phase_log_miss": incoming.get("phase_log_miss"),
                "phase_cumulative_probability": incoming.get("phase_cumulative_probability"),
            }
        self._tick_session_telemetry()
